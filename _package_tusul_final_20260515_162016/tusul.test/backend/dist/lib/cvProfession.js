const GENERIC_TARGET_RE = /^(software engineer|full[\s-]?stack|web developer|frontend|backend developer|developer|programmer|generalist|it specialist|программ|хөгжүүлэгч|инженер|оюутан|мэргэжилтэн|professional)$/i;
const STUDENT_MARKERS = /оюутан|суралцаж\s*буй|суралцаж\s*байна|undergraduate|college\s+student|university\s+student/i;
const DOMAIN_HINTS = [
...
const TITLE_LINE_RE = /нягтлан|accountant|developer|engineer|багш|nurse|дизайн|сувилгаан|маркетинг|программ|санхүү/i;
function countMatches(text, re) {
    return (text.match(new RegExp(re.source, re.flags.includes('i') ? 'gi' : 'g')) || []).length;
}
export function isStudentCv(cvText) {
    return (STUDENT_MARKERS.test(cvText) ||
        /\b[1-4][\s-]?р\s*курс\b/i.test(cvText) ||
        /\b\d+\s*р\s*ангийн?\s*оюутан/i.test(cvText));
}
function cleanStudyField(raw) {
    return raw
...
function extractSchoolLine(cvText) {
...
function extractYearLabel(cvText, lang) {
...
export function extractStudentStudyProfile(cvText, lang) {
    if (!isStudentCv(cvText))
        return null;
    const fieldPatterns = [
    ...
    for (const re of fieldPatterns) {
        ...
        if (raw.length < 3 || raw.length > 56)
            continue;
        if (/оюутан|student|суралцаж/i.test(raw))
            continue;
        fieldPlain = raw;
        break;
    }
...
export function isTechCv(cvText) {
    const t = cvText.toLowerCase();
    const hits = (t.match(/\b(react|node\.?js|typescript|javascript|python|java|sql|docker|aws)\b/g) || []).length;
    return hits >= 2;
}
export function isGenericTargetRole(role) {
    const t = role.trim();
    if (!t)
        return true;
    if (GENERIC_TARGET_RE.test(t))
        return true;
    if (/software engineer/i.test(t) && t.length < 40)
        return true;
    return false;
}
function professionMentionedInCv(role, cvText) {
...
export function inferProfessionFromCv(cvText, lang) {
    ...
    if (titleLine) {
        ...
        if (cleaned.length > 4 && !/оюутан\s*$/i.test(cleaned))
            return cleaned;
    }
...
export function resolveDisplayRole(targetRole, cvText, lang) {
    const trimmed = targetRole.trim().replace(/^[\d•\-\s]+/, '');
    const inferred = inferProfessionFromCv(cvText, lang);
    const looksLikeJobBlob = trimmed.length > 72 ||
        /^[•\-–]/.test(trimmed) ||
        (trimmed.match(/[•\n]/g) || []).length >= 2 ||
        /сонирхолтой|шинэ төгсөгч|чиглэлд\s*ажиллах|job\s+description|requirements/i.test(trimmed);
    if (!trimmed)
        return inferred || (lang === 'mn' ? 'Мэргэжилтэн' : 'Professional');
    if (looksLikeJobBlob)
        return inferred || trimmed.split(/\n/)[0].replace(/^[•\-\s]+/, '').slice(0, 56);
    if (isGenericTargetRole(trimmed))

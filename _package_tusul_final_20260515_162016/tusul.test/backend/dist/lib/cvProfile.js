...
export function parseCvProfile(raw) {
    ...
    const hasData = profile.fullName ||
        profile.phone ||
        profile.email ||
        profile.summary ||
...
function sectionHeader(key, lang) {
    const mn = {
        hobbies: 'СОНИРХОЛ',
        skills: 'УР ЧАДВАР',
        languages: 'ХЭЛ',
...
export function buildCvTextFromProfile(profile, lang) {
    const lines = [];
    const contact = [];
    if (profile.phone)
    ...
    if (profile.address)
        contact.push(profile.address);
    if (contact.length) {
        lines.push(lang === 'mn' ? 'ХОЛБОО БАРИХ' : 'CONTACT', ...contact, '');
    }
    const pushList = (key, items) => {
        if (!items.length)
            return;
        const h = sectionHeader(key, lang);
        if (h)
            lines.push(h, ...items.map((i) => `• ${i}`), '');
    };
    if (profile.summary)
        pushList('summary', [profile.summary]);
...
export function mergeAnalysisWithCvProfile(input, language) {

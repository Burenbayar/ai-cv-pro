const SECTION_HEADER_RE = /^(миний\s*тухай|холбоо\s*барих|contact|боловсрол|education|ажлын\s*туршлага|work\s*experience|ур\s*чадвар|skills|сонирхол)/i;
...
export function splitContactFragments(line) {
    ...
    if (email) {
        ...
        if (before.length > 6 && !SECTION_HEADER_RE.test(before))
            parts.push(before);
        parts.push(email);
    ...
    if (loc) {
        ...
        if (before.length > 6 && !/@/.test(before))
            parts.push(before);
        parts.push(loc.trim());
        t = t.replace(loc, '').trim();
    }
    if (t.length > 6 && !SECTION_HEADER_RE.test(t))
        parts.push(t);
...
export function parseContactFields(lines) {
    ...
    for (const line of lines)
        fragments.push(...splitContactFragments(line));
    for (const raw of fragments) {
        const line = raw.trim();
        if (!line || SECTION_HEADER_RE.test(line))
            continue;
        const emailHit = line.match(EMAIL_RE)?.[0];
        if (emailHit && !email) {
            email = emailHit;
            const after = line.slice(line.indexOf(emailHit) + emailHit.length).trim();
            if (after && LOCATION_RE.test(after))
                location = after.match(LOCATION_RE)?.[0]?.trim() || after;
            else if (after.length > 6 && !PHONE_RE.test(after))
                rest.push(after);
            continue;
        }

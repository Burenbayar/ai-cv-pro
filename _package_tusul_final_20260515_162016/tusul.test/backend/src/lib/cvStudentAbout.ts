...
import type {StudentStudyProfile} from './cvProfession.js';

type StudentAboutOpts = {
  cvText: string;
  careerGoals?: string;
  language: CvLanguage;
  narratives: string[];
...
function pickGoalSentence(opts: StudentAboutOpts): string {
  const goals = opts.careerGoals?.trim();
  if (goals && goals.length > 12) return goals.endsWith('.') ? goals : `${goals}.`;

  const fromCv = opts.narratives.find((n) => /зорилго|сонирхол|чиглэл|хүсэлтэй|aspir|looking to/i.test(n));
  if (fromCv) return fromCv.endsWith('.') ? fromCv : `${fromCv}.`;
...
export function buildStudentProfessionalAbout(profile: StudentStudyProfile, opts: StudentAboutOpts): string {
